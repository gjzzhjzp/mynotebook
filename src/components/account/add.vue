            ...
            <view class=" account-add-container flex-column-left flex-justify-between" style="height: 100%;">
                ...
                <view class="l-account-add p-a-20">
                    <view class="flex-align-center flex-justify-between">
                        ...
                        <nut-calendar
                            v-model:visible="showCalender"
                            :default-value="formData.accountDate"
                            start-date="2022-01-11"
                            end-date="2022-11-30"
                            @close="showCalender = false"
                            @choose="chooseDate"
                        >
                        </nut-calendar>
                        <view class="flex-align-center">
                            <view v-for="item in types" :key="item.title" class="m-r-10"
                                ...
                                <nut-button :type="item.value != checked_type ? 'default' : 'primary'"
                                    size="small">{{ item.title }}</nut-button>
                            </view>
                        </view>
                    </view>
                    <view class="m-t-10">
                            ...
                            <nut-grid-item  v-for="item in categories" :key="item.value">
                                <template #default> -->
                                    <view v-for="item in categories" class="m-r-10 m-t-10">
                                        <nut-button :type="item.value != checked_category ? 'default' : 'primary'"
                                        size="small" @click="checked_category = item.value" style="width: 145rpx;">{{item.title}}</nut-button>
                                    </view>
